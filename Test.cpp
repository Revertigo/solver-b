...
TEST_CASE("Check the correctness of subtraction"){
    ...
    CHECK_LT(abs(solve(y-y-2-y-(y + 5i) == y-y-2-y-(y + 5i) ).imag()), EPSILON);//y.img = 0
    CHECK_LT(abs(solve(y-y-2-y-(y + 5i) == y-y-2-y-(y + 5i) ).real()), EPSILON);//y.real = 0
}
...
TEST_CASE("Check the correctness of multiplication"){
    ...
    CHECK_LT(abs(solve(4*y+15i == y + 8i + 3*y).imag() -7), EPSILON);//y.imag = 7i
    CHECK_LT(abs(solve(4*y+15i == y + 8i + 3*y).real()), EPSILON);//y.real = 0
...
TEST_CASE("Check the correctness of division"){
    ...
    ComplexVariable y;
    CHECK_LT(abs(solve(y/3 == (1/3)).real() -1.0), EPSILON);//y.real = 1
    CHECK_LT(abs(solve(y/3 == (1/3)).imag()), EPSILON);//y.imag = 0
    CHECK_LT(abs(solve(y/2/3/4/5 +5i == (1/2/3/4/5)).real() -1.0), EPSILON);//y.real = 1
    CHECK_LT(abs(solve(y/2/3/4/5 +5i == (1/2/3/4/5)).imag() -5), EPSILON);//y.imag = 5i
    CHECK_THROWS(abs(solve(y/0 == 0)));//throw exception
    CHECK_THROWS(abs(solve(y/5/0 == 5)));//throw exception
}
...
TEST_CASE("Check the correctness of equality"){
    ...
    CHECK_LT(abs(solve((y^2) == (y^2)).real()), EPSILON);//y.real = 0
    CHECK_LT(abs(solve((y^2) == (y^2)).imag()), EPSILON);//y.imag = 0
    CHECK_LT(abs(solve(3*y -(y + 6i) == 3*y -(y + 6i)).real()), EPSILON);//y.real = 0
    CHECK_LT(abs(solve(3*y -(y + 6i) == 3*y -(y + 6i) ).imag()), EPSILON);//y.imag = 0
}
...
TEST_CASE("Check the correctness of all operators together"){
    ...
    CHECK_LT(abs(solve(3*y-y-y-y-y+y+y+y+y -4 == 10).real() - 7.0), EPSILON); // y.real = 7
    CHECK_LT(abs(solve(3*y-y-y-y-y+y+y+y+y - 4 == 10).imag()), EPSILON); // y.imag = 0
    CHECK_LT(abs(solve(3*y-(y +1i) -(y + 1i) -4 == 3*y + 15i).real() - 7.0), EPSILON); // y.real = -2
    CHECK_LT(abs(solve(3*y-(y +1i) -(y + 1i) -4 == 3*y + 15i).imag() - 7.0), EPSILON); // y.imag = -17i
    check = (((abs(solve(4*(y^2) + 8*y +5 == 0).imag()*5 -3)) < EPSILON) ||
             ((abs(solve(4*(y^2) + 8*y +5 == 0).imag()*5 +3)) < EPSILON));
    CHECK(check);// y.imag = 3i/5 or -3i/5
    CHECK_LT(abs(solve(4*(y^2) + 8*y +5 == 0).real() +4.0), EPSILON);//y.real = -4
    CHECK_LT(abs(solve(2*y/4 + 3*y -(6*y+1i) == 2*y/4 + 3*y - (6*y + 2i)).real()), EPSILON);//y.real = 0
    CHECK_LT(abs(solve(2*y/4 + 3*y -(6*y+1i) == 2*y/4 + 3*y - (6*y + 2i)).imag() +1), EPSILON);//y.imag = -i
    CHECK_LT(abs(solve((y^2)/4 +3*y + 4i == (y^2) + 2i).imag() -2i), EPSILON);//y.imag = 1i
    CHECK_LT(abs(solve((y^2)/4 +3*y + 4i == -1*(y^2) + 2i).real()), EPSILON);//y.real = 0
    check = (((abs(solve((y^2)/4 + y +5/4 == 0).imag() -1i)) < EPSILON) ||
             ((abs(solve((y^2)/4 + y +5/4 == 0).imag() -1i)) < EPSILON));
    CHECK(check);// y.imag = i or -i
    CHECK_LT(abs(solve((y^2)/4 +y + 5/4 == 0).real() +2.0), EPSILON);//y.real = -2.0
    CHECK_LT(abs(solve(y-2 + 3*y == y-3 + 2*y).real() +0.25), EPSILON);//y.real = -0.25
